...
public class JacksonJsonValueResolverTest {
	...
	public void testMapToType() throws Exception {
		...
		InputStream from = new ByteArrayInputStream(String.valueOf(
				"{\"name\":\"snowgoose\",\"alive\":true,\"date\":"
						+ expectedDate.getTime() + "}").getBytes());
		when(requestContext.getRequestBody()).thenReturn(from);
		Bean actual = (Bean) mapper.resolveValue(requestContext, null, null,
				Bean.class, null);
	...
	public void testSupports() throws Exception {
		...
		assertThat(mapper.supports(MediaTypes.valueOf("text/javascript")),
				is(false));
		assertThat(
				mapper.supports(MediaTypes.valueOf("aplication/javascript")),
				is(false));
	}
}
